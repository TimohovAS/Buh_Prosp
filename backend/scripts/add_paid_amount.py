...
async def run():
    async with engine.begin() as conn:
        ...
        if "paid_amount" not in columns:
            await conn.execute(text("ALTER TABLE income ADD COLUMN paid_amount FLOAT DEFAULT 0.0"))
        ...
        await conn.execute(text("UPDATE income SET paid_amount = 0.0 WHERE paid_amount IS NULL"))
