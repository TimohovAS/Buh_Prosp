...
async def main():
    async with SessionLocal() as db:
        ...
        migrated_incomes = 0
        for inc in incomes:
            r_exists = await db.execute(select(BankTransaction).where(BankTransaction.matched_type == "income", BankTransaction.matched_id == inc.id))
        ...
        migrated_expenses = 0
        for exp in expenses:
            r_exists = await db.execute(select(BankTransaction).where(BankTransaction.matched_type == "expense", BankTransaction.matched_id == exp.id))
            if r_exists.scalar_one_or_none():
                continue
