...
def ensure_column(cursor: sqlite3.Cursor, table: str, column: str, definition: str) -> None:
    if not table_exists(cursor, table):
        print(f"[v7] Table {table} not found, skipping {column}.")
        return
    if has_column(cursor, table, column):
        print(f"[v7] Column {table}.{column} already exists.")
        return
    cursor.execute(f"ALTER TABLE {table} ADD COLUMN {column} {definition}")
    print(f"[v7] Added column {table}.{column}.")


def main() -> None:
    print(f"[v7] Using DB: {DB_PATH}")
    DB_PATH.parent.mkdir(parents=True, exist_ok=True)
    conn = sqlite3.connect(DB_PATH)
    try:
        cursor = conn.cursor()
        ensure_column(cursor, "enterprise", "emblem_data_url", "TEXT")
        conn.commit()
        print("[v7] Done.")
    finally:
        conn.close()
