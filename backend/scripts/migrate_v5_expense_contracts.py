"""Migration v5: add contract link for expenses."""
...
def ensure_column(cursor: sqlite3.Cursor, table: str, column: str, definition: str) -> None:
    if not table_exists(cursor, table):
        print(f"[v5] Table {table} not found, skipping {column}.")
        return
    if has_column(cursor, table, column):
        print(f"[v5] Column {table}.{column} already exists.")
        return
    cursor.execute(f"ALTER TABLE {table} ADD COLUMN {column} {definition}")
    print(f"[v5] Added column {table}.{column}.")


def main() -> None:
    print(f"[v5] Using DB: {DB_PATH}")
    DB_PATH.parent.mkdir(parents=True, exist_ok=True)
    conn = sqlite3.connect(DB_PATH)
    try:
        cursor = conn.cursor()
        ensure_column(cursor, "expenses", "contract_id", "INTEGER")
        conn.commit()
        print("[v5] Done.")
    finally:
        conn.close()
