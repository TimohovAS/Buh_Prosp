...
export default function useCategoryProjectResolver(categories, lang = 'ru') {
  const getCategoryById = useCallback(
    (categoryId) => categories.find((item) => String(item.id) === String(categoryId)) || null,
    [categories]
  )

  const getCategoryDefaultProjectId = useCallback(
    (categoryId) => {
  ...
  const usesCategoryProject = useCallback(
    (categoryId) => Boolean(getCategoryDefaultProjectId(categoryId)),
    [getCategoryDefaultProjectId]
  )

  const getCategoryLabel = useCallback(
    (categoryOrId, emptyLabel = UI_DASH) => {
      const category =
        typeof categoryOrId === 'object' && categoryOrId !== null
