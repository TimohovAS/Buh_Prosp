...
export default function Projects() {
  ...
  const handleExportPurchasesXlsx = async () => {
    ...
    const safeName =
      (purchaseModal.project_name || `project_${purchaseModal.project_id}`)
        .replace(/[^\w-]+/g, '_')
        .replace(/^_+|_+$/g, '') || `project_${purchaseModal.project_id}`
    const filename = `purchases_${safeName}_${range.from}_${range.to}.xlsx`
    try {
      await api.receipts.exportByProjectXlsx(
        purchaseModal.project_id,
        {
          from: range.from,
          to: range.to,
          lang: getLang(),
            ...
            <thead>
              <tr>
                <th style={{ cursor: 'pointer' }} onClick={() => toggleSort('name')}>
                  {tr('project')} <SortIndicator active={sortCol === 'name'} asc={sortAsc} />
                </th>
                <th style={{ cursor: 'pointer' }} onClick={() => toggleSort('code')}>
                  {tr('projectCode')} <SortIndicator active={sortCol === 'code'} asc={sortAsc} />
                </th>
                ...
                <th style={{ cursor: 'pointer' }} onClick={() => toggleSort('revenue')}>
                  {tr('income')} <SortIndicator active={sortCol === 'revenue'} asc={sortAsc} />
                </th>
                <th style={{ cursor: 'pointer' }} onClick={() => toggleSort('expenses')}>
                  {tr('expenses')} <SortIndicator active={sortCol === 'expenses'} asc={sortAsc} />
                </th>
                <th style={{ cursor: 'pointer' }} onClick={() => toggleSort('profit')}>
                  {tr('projectProfit')} <SortIndicator active={sortCol === 'profit'} asc={sortAsc} />
                </th>
              </tr>
            </thead>
            <tbody>
            ...
            <div className="record-field-grid record-field-grid-wide">
              ...
              <div className="record-field full">
                ...
                <span
                  ...
                  style={{
                    color:
                      (getRowData(detailModal).profit ?? 0) >= 0
                        ? 'var(--color-success)'
                        : 'var(--color-danger)',
                    fontWeight: 700,
                  }}
          ...
          detailModal ? (
            <div className="record-actions-grid">
              <button
                ...
                onClick={() => openMovementsFromDetail(detailModal)}
              >
                {tr('projectMovements')}
              </button>
              <button
                ...
                onClick={() => openPurchasesFromDetail(detailModal)}
              >
                {tr('projectPurchases')}
              </button>
              <button
          ...
          <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
            ...
            >
              ...
              {movementPeriodQuick === 'custom' ? (
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                  <DatePicker
                    value={movementCustomFrom}
                    onChange={setMovementCustomFrom}
                    placeholder={tr('periodFrom')}
                    className="form-input"
                  />
                  <span>{UI_DASH}</span>
                  <DatePicker
                    value={movementCustomTo}
                    onChange={setMovementCustomTo}
                    placeholder={tr('periodTo')}
                    className="form-input"
                  />
                </div>
              ...
              <div style={{ color: 'var(--color-text-muted)', fontSize: '0.9rem' }}>
                {movementModal.from_date} {UI_DASH} {movementModal.to_date} ·{' '}
                {tr(movementModal.mode === 'cash' ? 'financeModeCash' : 'financeModeAccrual')}
              </div>
            </div>
            {movementError ? (
              ...
              {purchasePeriodQuick === 'custom' ? (
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                  <DatePicker
                    ...
                    onChange={setPurchaseCustomFrom}
                    placeholder={tr('periodFrom')}
                    className="form-input"
                  />
                  <span>{UI_DASH}</span>
                    ...
                    {purchaseModal.items.map((item) => (
                      ...
                      >
                        ...
                        <td style={{ textAlign: 'right', fontWeight: 700 }}>
                          {formatMoney2(item.total_amount || 0)} RSD
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <div className="form-group">
              <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                <input
                  ...
                  onChange={(event) => setForm({ ...form, is_internal: event.target.checked })}
                />
