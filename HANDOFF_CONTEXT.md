...
- В секции `Собственные средства` на странице `Займы` есть агрегированные итоги `Внесено`, `Возвращено` и `Фирма должна владельцу`, плюс итоговая строка под таблицей. Это нужно, чтобы возвраты собственных средств были видны и сразу был понятен текущий остаток долга фирмы владельцу.
- Таблица `Собственные средства` намеренно компактная: одна строка на движение, видимые колонки `Дата`, `Тип`, `Описание`, `Сумма`; итоги `Внесено`/`Возвращено` показываются только в верхних карточках, без дублирования в строках. Длинный контрагент, счёт и банковский референс спрятаны в tooltip строки и доступны в карточке по клику.
